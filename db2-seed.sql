--------------------Stykke----------------------------
INSERT INTO SesongType (ID, Navn)
VALUES
(1, 'Vinter'),
(2, 'Vår'),
(3, 'Sommer'),
(4, 'Høst');

INSERT INTO Stykke (ID, Navn)
VALUES
(1, 'Kongsemnene'),
(2, 'Størst av alt er kjærligheten');

-- SesongStykke Table
INSERT INTO SesongStykke (SesongTypeID, År, StykkeID, SalID)
VALUES
(1, 2024, 1, 1), -- Kongsemnene Vinter 2024
(1, 2024, 2, 2), -- Størst av alt er kjærligheten Vinter 2024
(2, 2024, 1, 1), -- Kongsemnene Vår 2024
(2, 2024, 2, 2); -- Størst av alt er kjærligheten Vår 2024

-------------Akt og Rolle----------------
-- Akt Table
INSERT INTO Akt ( StykkeID, AktNR, Navn)
VALUES
--Akter i Kongsemnene
(1, 1, 'Akt 1 - Kongsemnene'),
(1, 2, 'Akt 2 - Kongsemnene'),
(1, 3, 'Akt 3 - Kongsemnene'),
(1, 4, 'Akt 4 - Kongsemnene'),
(1, 5, 'Akt 5 - Kongsemnene'),
--Bare en akt i Størst av alt er kjærligheten
(2, 1, 'Akt 1 - Størst av alt er kjærligheten');

-- Rolle Table
INSERT INTO Rolle (ID, RolleNavn)
VALUES
--Roller i Kongsemnene
(1, 'Håkon Håkonsson'),
(2, 'Inga frå Vartejg'),
(3, 'Skule Jarl'),
(4, 'Fru Ragnhild'),
(5, 'Margrete'),
(6, 'Sigrid'),
(7, 'Ingebjørg'),
(8, 'Biskop Nikolas'),
(9, 'Gregorius Jonssønn'),
(10, 'Paal Flida'),
(11, 'Trønder'),
(12, 'Baard Bratte'),
(13, 'Jatgeir Skald'),
(14, 'Dagfinn Bonde'),
(15, 'Peter'),
-- Roller i Størst av alt er kjærligheten
(16, 'Sunniva Du Mond Nordal'),
(17, 'Jo Saberniak'),
(18, 'Marte M. Steinholt'),
(19, 'Tor Ivar Hagen'),
(20, 'Trond-Ove Skrødal'),
(21, 'Natalie Grøndahl Tangen'),
(22, 'Åsmund Flaten');

-- AktRolle Table
INSERT INTO AktRolle (StykkeID, AktNR, RolleID, AnsattID)
VALUES
-----Størst av alt er kjærligheten-----
-----Skuespillerne i Størst av alt er kjærligheten spiller seg selv
(2,1, 16, 13),
(2,1, 17, 14),
(2,1, 18, 15),
(2,1, 19, 16),
(2,1, 20, 17),
(2,1, 21, 18),
(2,1, 22, 19),
-----Kongsemnene-----
-- For Håkon Håkonsson
(1,1, 1, 1),
(1,2, 1, 1),
(1,3, 1, 1),
(1,4, 1, 1),
(1,5, 1, 1),
-- For Dagfinn Bonde
(1,1, 14, 11),
(1,2, 14, 11),
(1,3, 14, 11),
(1,4, 14, 11),
(1,5, 14, 11),

-- For Jatgeir Skald
(1,4, 13, 11),

-- For Sigrid
(1,1, 6, 6),
(1,2, 6, 6),
(1,5, 6, 6),

-- For Ingeborg
(1,4, 7, 6),

-- For Skule Jarl
(1,1, 3, 3),
(1,2, 3, 3),
(1,3, 3, 3),
(1,4, 3, 3),
(1,5, 3, 3),

-- For Inga fra Varteig
(1,1, 2, 2),
(1,3, 2, 2),

-- For Paal Flida
(1,1, 10, 9),
(1,2, 10, 9),
(1,3, 10, 9),
(1,4, 10, 9),
(1,5, 10, 9),

-- For Ragnhild
(1,1, 4, 4),
(1,5, 4, 4),

-- For Gregorius Jonsson
(1,1, 9, 8),
(1,2, 9, 8),
(1,3, 9, 8),
(1,4, 9, 8),
(1,5, 9, 8),

-- For Margrete
(1,1, 5, 5),
(1,2, 5, 5),
(1,3, 5, 5),
(1,4, 5, 5),
(1,5, 5, 5),

-- For Biskop Nikolas
(1,1, 8, 7),
(1,2, 8, 7),
(1,3, 8, 7),

-- For Peter
(1,3, 15, 12),
(1,4, 15, 12),
(1,5, 15, 12);


----------------------Forestilling-------------------------
-- Sal Table
INSERT INTO Sal (ID, Navn)
VALUES 
(1, 'Hovedscenen'),
(2, 'Gamle scene')/* ,
(3, 'Studioscenen'),
(4, 'Teaterkjelleren'),
(5, 'Teaterkafeen') */;

-- Forestilling Table
INSERT INTO Forestilling (SalID, StartTid, SesongTypeID, SesongStykkeÅr)
VALUES 
--Kongemnene
(1,'2024-02-01 19:00:00', 1, 2024),
(1,'2024-02-02 19:00:00', 1, 2024),
(1,'2024-02-03 19:00:00', 1, 2024),
(1,'2024-02-05 19:00:00', 1, 2024),
(1,'2024-02-06 19:00:00', 1, 2024),
--Størst av alt er kjærligheten
(2,'2024-02-03 18:30:00', 1, 2024),
(2,'2024-02-06 18:30:00', 1, 2024),
(2,'2024-02-07 18:30:00', 1, 2024),
(2,'2024-02-12 18:30:00', 1, 2024),
(2,'2024-02-13 18:30:00', 1, 2024),
(2,'2024-02-14 18:30:00', 1, 2024);

-- Område Table
INSERT INTO Område (SalID, Navn)
VALUES
(1, "Parkett"),
(1, "Galleri"),
(2, "Galleri"),
(2, "Balkong"),
(2, "Parkett");

-- AnsattStatus Table
INSERT INTO AnsattStatus (ID, Navn)
VALUES
(1, 'Fast ansatt'),
(2, 'Midlertidig ansatt'),
(3, 'Innleid'),
(4, 'Frivillig');

-- Posisjon Table
INSERT INTO AnsattPosisjon (ID, Navn)
VALUES
(1, 'Skuespiller'),
(2, 'Regi'),
(3, 'Scenografi og kostymer'),
(4, 'Musikalsk ansvarlig'),
(5, 'Lysdesign'),
(6, 'Dramaturg');

-- Ansatt Table
INSERT INTO Ansatt (ID, AnsattStatusID, AnsattPosisjonID, Navn, Epost)
VALUES

--Kongsemene - Skuespillere
(1, 1, 1, 'Arturo Scotti', 'arturo.scotti@email.com'),
(2, 1, 1, 'Ingunn Beate Øyen', 'ingunn.beate.oyen@email.com'),
(3, 1, 1, 'Hans Petter Nilsen', 'hans.petter.nilsen@email.com'),
(4, 1, 1, 'Madeleine Brandtzæg Nilsen', 'madeleine.brandtzaeg.nilsen@email.com'),
(5, 1, 1, 'Synnøve Fossum Eriksen', 'synnove.fossum.eriksen@email.com'),
(6, 1, 1, 'Emma Caroline Deichmann', 'emma.caroline.deichmann@email.com'),
(7, 1, 1, 'Thomas Jensen Takyi', 'thomas.jensen.takyi@email.com'),
(8, 1, 1, 'Per Bogstad Gulliksen', 'per.bogstad.gulliksen@email.com'),
(9, 1, 1, 'Isak Holmen Sørensen', 'isak.holmen.sorensen@email.com'),
(10, 1, 1, 'Fabian Heidelberg Lunde', 'fabian.heidelberg.lunde@email.com'),
(11, 1, 1, 'Emil Olafsson', 'emil.olafsson@email.com'),
(12, 1, 1, 'Snorre Ryen Tøndel', 'snorre.ryen.tondel@email.com'),
--Størst av alt er kjærligheten - Skuespillere
(13, 1, 1, 'Sunniva Du Mond Nordal', 'sunniva.du.mond.nordal@email.com'),
(14, 1, 1, 'Jo Saberniak', 'jo.saberniak@email.com'),
(15, 1, 1, 'Marte M. Steinholt', 'marte.m.steinholt@email.com'),
(16, 1, 1, 'Tor Ivar Hagen', 'tor.ivar.hagen@email.com'),
(17, 1, 1, 'Trond-Ove Skrødal', 'trond-ove.skrodal@email.com'),
(18, 1, 1, 'Natalie Grøndahl Tangen', 'natalie.grondahl.tangen@email.com'),
(19, 1, 1, 'Åsmund Flaten', 'asmund.flaten@email.com'),
--Kongsemnene - Regi
(20, 1, 2, 'Yury Butusov', 'yury.butusov@email.com'),
(21, 1, 3, 'Aleksandr Shishkin-Hokusai', 'aleksandr.shishkinhokusai@email.com'),
(22, 1, 5, 'Eivind Myren', 'eivind.myren@email.com'),
(23, 1, 6, 'Mina Rype Stokke', 'mina.stokke@email.com'),
--Størst av alt er kjærligheten - Regi
(24, 1, 2, 'Jonas Corell Petersen', 'jonascorell.petersen@email.com'),
(25, 1, 3, 'David Gehrt', 'david.gehrt@email.com'),
(26, 1, 4, 'Gaute Tønder', 'gaute.tonder@email.com'),
(27, 1, 5, 'Magnus Mikaelsen', 'magnus.mikaelsen@email.com'),
(28, 1, 6, 'Kristoffer Spender', 'kristoffer.spender@email.com');

-- AnsattOppgave Table
-----------------------------------------------------------------------------------------------------

-- INSERT INTO Stol (SalID, OmrådeNavn, StolNR, RadNR)
-- VALUES
-- --Hovedscene Område 'Hoved' Rad 1
-- (1, "Parkett", 1, 1), 
-- (1, "Parkett", 2, 1), 
-- (1, "Parkett", 3, 1), 
-- (1, "Parkett", 4, 1), 
-- (1, "Parkett", 5, 1), 
-- (1, "Parkett", 6, 1), 
-- (1, "Parkett", 7, 1), 
-- (1, "Parkett", 8, 1), 
-- (1, "Parkett", 9, 1), 
-- (1, "Parkett", 10, 1),
-- (1, "Parkett", 11, 1), 
-- (1, "Parkett", 12, 1), 
-- (1, "Parkett", 13, 1), 
-- (1, "Parkett", 14, 1), 
-- (1, "Parkett", 15, 1), 
-- (1, "Parkett", 16, 1), 
-- (1, "Parkett", 17, 1), 
-- (1, "Parkett", 18, 1), 
-- (1, "Parkett", 19, 1), 
-- (1, "Parkett", 20, 1),
-- (1, "Parkett", 21, 1), 
-- (1, "Parkett", 22, 1), 
-- (1, "Parkett", 23, 1), 
-- (1, "Parkett", 24, 1), 
-- (1, "Parkett", 25, 1), 
-- (1, "Parkett", 26, 1), 
-- (1, "Parkett", 27, 1), 
-- (1, "Parkett", 28, 1),
-- --Hovedscene Område 'Hoved' Rad 2
-- (1, "Parkett", 29, 2), 
-- (1, "Parkett", 30, 2), 
-- (1, "Parkett", 31, 2), 
-- (1, "Parkett", 32, 2), 
-- (1, "Parkett", 33, 2), 
-- (1, "Parkett", 34, 2), 
-- (1, "Parkett", 35, 2), 
-- (1, "Parkett", 36, 2), 
-- (1, "Parkett", 37, 2), 
-- (1, "Parkett", 38, 2),
-- (1, "Parkett", 39, 2), 
-- (1, "Parkett", 40, 2), 
-- (1, "Parkett", 41, 2), 
-- (1, "Parkett", 42, 2), 
-- (1, "Parkett", 43, 2), 
-- (1, "Parkett", 44, 2), 
-- (1, "Parkett", 45, 2), 
-- (1, "Parkett", 46, 2), 
-- (1, "Parkett", 47, 2), 
-- (1, "Parkett", 48, 2),
-- (1, "Parkett", 49, 2), 
-- (1, "Parkett", 50, 2), 
-- (1, "Parkett", 51, 2), 
-- (1, "Parkett", 52, 2), 
-- (1, "Parkett", 53, 2), 
-- (1, "Parkett", 54, 2), 
-- (1, "Parkett", 55, 2), 
-- (1, "Parkett", 56, 2),
-- --Hovedscene Område 'Hoved' Rad 3
-- (1, "Parkett", 57, 3),
-- (1, "Parkett", 58, 3),
-- (1, "Parkett", 59, 3),
-- (1, "Parkett", 60, 3),
-- (1, "Parkett", 61, 3),
-- (1, "Parkett", 62, 3),
-- (1, "Parkett", 63, 3),
-- (1, "Parkett", 64, 3),
-- (1, "Parkett", 65, 3),
-- (1, "Parkett", 66, 3),
-- (1, "Parkett", 67, 3),
-- (1, "Parkett", 68, 3),
-- (1, "Parkett", 69, 3),
-- (1, "Parkett", 70, 3),
-- (1, "Parkett", 71, 3),
-- (1, "Parkett", 72, 3),
-- (1, "Parkett", 73, 3),
-- (1, "Parkett", 74, 3),
-- (1, "Parkett", 75, 3),
-- (1, "Parkett", 76, 3),
-- (1, "Parkett", 77, 3),
-- (1, "Parkett", 78, 3),
-- (1, "Parkett", 79, 3),
-- (1, "Parkett", 80, 3),
-- (1, "Parkett", 81, 3),
-- (1, "Parkett", 82, 3),
-- (1, "Parkett", 83, 3),
-- (1, "Parkett", 84, 3),
-- --Hovedscene Område 'Hoved' Rad 4
-- (1, "Parkett", 85, 4),
-- (1, "Parkett", 86, 4),
-- (1, "Parkett", 87, 4),
-- (1, "Parkett", 88, 4),
-- (1, "Parkett", 89, 4),
-- (1, "Parkett", 90, 4),
-- (1, "Parkett", 91, 4),
-- (1, "Parkett", 92, 4),
-- (1, "Parkett", 93, 4),
-- (1, "Parkett", 94, 4),
-- (1, "Parkett", 95, 4),
-- (1, "Parkett", 96, 4),
-- (1, "Parkett", 97, 4),
-- (1, "Parkett", 98, 4),
-- (1, "Parkett", 99, 4),
-- (1, "Parkett", 100, 4),
-- (1, "Parkett", 101, 4),
-- (1, "Parkett", 102, 4),
-- (1, "Parkett", 103, 4),
-- (1, "Parkett", 104, 4),
-- (1, "Parkett", 105, 4),
-- (1, "Parkett", 106, 4),
-- (1, "Parkett", 107, 4),
-- (1, "Parkett", 108, 4),
-- (1, "Parkett", 109, 4),
-- (1, "Parkett", 110, 4),
-- (1, "Parkett", 111, 4),
-- (1, "Parkett", 112, 4),
-- --Hovedscene Område '' Rad 5
-- (1, "Parkett", 113, 5),
-- (1, "Parkett", 114, 5),
-- (1, "Parkett", 115, 5),
-- (1, "Parkett", 116, 5),
-- (1, "Parkett", 117, 5),
-- (1, "Parkett", 118, 5),
-- (1, "Parkett", 119, 5),
-- (1, "Parkett", 120, 5),
-- (1, "Parkett", 121, 5),
-- (1, "Parkett", 122, 5),
-- (1, "Parkett", 123, 5),
-- (1, "Parkett", 124, 5),
-- (1, "Parkett", 125, 5),
-- (1, "Parkett", 126, 5),
-- (1, "Parkett", 127, 5),
-- (1, "Parkett", 128, 5),
-- (1, "Parkett", 129, 5),
-- (1, "Parkett", 130, 5),
-- (1, "Parkett", 131, 5),
-- (1, "Parkett", 132, 5),
-- (1, "Parkett", 133, 5),
-- (1, "Parkett", 134, 5),
-- (1, "Parkett", 135, 5),
-- (1, "Parkett", 136, 5),
-- (1, "Parkett", 137, 5),
-- (1, "Parkett", 138, 5),
-- (1, "Parkett", 139, 5),
-- (1, "Parkett", 140, 5),
-- --Hovedscene Område '' Rad 6
-- (1, "Parkett", 141, 6),
-- (1, "Parkett", 142, 6),
-- (1, "Parkett", 143, 6),
-- (1, "Parkett", 144, 6),
-- (1, "Parkett", 145, 6),
-- (1, "Parkett", 146, 6),
-- (1, "Parkett", 147, 6),
-- (1, "Parkett", 148, 6),
-- (1, "Parkett", 149, 6),
-- (1, "Parkett", 150, 6),
-- (1, "Parkett", 151, 6),
-- (1, "Parkett", 152, 6),
-- (1, "Parkett", 153, 6),
-- (1, "Parkett", 154, 6),
-- (1, "Parkett", 155, 6),
-- (1, "Parkett", 156, 6),
-- (1, "Parkett", 157, 6),
-- (1, "Parkett", 158, 6),
-- (1, "Parkett", 159, 6),
-- (1, "Parkett", 160, 6),
-- (1, "Parkett", 161, 6),
-- (1, "Parkett", 162, 6),
-- (1, "Parkett", 163, 6),
-- (1, "Parkett", 164, 6),
-- (1, "Parkett", 165, 6),
-- (1, "Parkett", 166, 6),
-- (1, "Parkett", 167, 6),
-- (1, "Parkett", 168, 6),
-- --Hovedscene Område '' Rad 7
-- (1, "Parkett", 169, 7),
-- (1, "Parkett", 170, 7),
-- (1, "Parkett", 171, 7),
-- (1, "Parkett", 172, 7),
-- (1, "Parkett", 173, 7),
-- (1, "Parkett", 174, 7),
-- (1, "Parkett", 175, 7),
-- (1, "Parkett", 176, 7),
-- (1, "Parkett", 177, 7),
-- (1, "Parkett", 178, 7),
-- (1, "Parkett", 179, 7),
-- (1, "Parkett", 180, 7),
-- (1, "Parkett", 181, 7),
-- (1, "Parkett", 182, 7),
-- (1, "Parkett", 183, 7),
-- (1, "Parkett", 184, 7),
-- (1, "Parkett", 185, 7),
-- (1, "Parkett", 186, 7),
-- (1, "Parkett", 187, 7),
-- (1, "Parkett", 188, 7),
-- (1, "Parkett", 189, 7),
-- (1, "Parkett", 190, 7),
-- (1, "Parkett", 191, 7),
-- (1, "Parkett", 192, 7),
-- (1, "Parkett", 193, 7),
-- (1, "Parkett", 194, 7),
-- (1, "Parkett", 195, 7),
-- (1, "Parkett", 196, 7),
-- -- Hovedscene Område d' Rad 8
-- (1, "Parkett", 197, 8),
-- (1, "Parkett", 198, 8),
-- (1, "Parkett", 199, 8),
-- (1, "Parkett", 200, 8),
-- (1, "Parkett", 201, 8),
-- (1, "Parkett", 202, 8),
-- (1, "Parkett", 203, 8),
-- (1, "Parkett", 204, 8),
-- (1, "Parkett", 205, 8),
-- (1, "Parkett", 206, 8),
-- (1, "Parkett", 207, 8),
-- (1, "Parkett", 208, 8),
-- (1, "Parkett", 209, 8),
-- (1, "Parkett", 210, 8),
-- (1, "Parkett", 211, 8),
-- (1, "Parkett", 212, 8),
-- (1, "Parkett", 213, 8),
-- (1, "Parkett", 214, 8),
-- (1, "Parkett", 215, 8),
-- (1, "Parkett", 216, 8),
-- (1, "Parkett", 217, 8),
-- (1, "Parkett", 218, 8),
-- (1, "Parkett", 219, 8),
-- (1, "Parkett", 220, 8),
-- (1, "Parkett", 221, 8),
-- (1, "Parkett", 222, 8),
-- (1, "Parkett", 223, 8),
-- (1, "Parkett", 224, 8),
-- -- Hovedscene Område d' Rad 9
-- (1, "Parkett", 225, 9),
-- (1, "Parkett", 226, 9),
-- (1, "Parkett", 227, 9),
-- (1, "Parkett", 228, 9),
-- (1, "Parkett", 229, 9),
-- (1, "Parkett", 230, 9),
-- (1, "Parkett", 231, 9),
-- (1, "Parkett", 232, 9),
-- (1, "Parkett", 233, 9),
-- (1, "Parkett", 234, 9),
-- (1, "Parkett", 235, 9),
-- (1, "Parkett", 236, 9),
-- (1, "Parkett", 237, 9),
-- (1, "Parkett", 238, 9),
-- (1, "Parkett", 239, 9),
-- (1, "Parkett", 240, 9),
-- (1, "Parkett", 241, 9),
-- (1, "Parkett", 242, 9),
-- (1, "Parkett", 243, 9),
-- (1, "Parkett", 244, 9),
-- (1, "Parkett", 245, 9),
-- (1, "Parkett", 246, 9),
-- (1, "Parkett", 247, 9),
-- (1, "Parkett", 248, 9),
-- (1, "Parkett", 249, 9),
-- (1, "Parkett", 250, 9),
-- (1, "Parkett", 251, 9),
-- (1, "Parkett", 252, 9),
-- -- Hovedscene Område 'Hoved' Rad 10
-- (1, "Parkett", 253, 10),
-- (1, "Parkett", 254, 10),
-- (1, "Parkett", 255, 10),
-- (1, "Parkett", 256, 10),
-- (1, "Parkett", 257, 10),
-- (1, "Parkett", 258, 10),
-- (1, "Parkett", 259, 10),
-- (1, "Parkett", 260, 10),
-- (1, "Parkett", 261, 10),
-- (1, "Parkett", 262, 10),
-- (1, "Parkett", 263, 10),
-- (1, "Parkett", 264, 10),
-- (1, "Parkett", 265, 10),
-- (1, "Parkett", 266, 10),
-- (1, "Parkett", 267, 10),
-- (1, "Parkett", 268, 10),
-- (1, "Parkett", 269, 10),
-- (1, "Parkett", 270, 10),
-- (1, "Parkett", 271, 10),
-- (1, "Parkett", 272, 10),
-- (1, "Parkett", 273, 10),
-- (1, "Parkett", 274, 10),
-- (1, "Parkett", 275, 10),
-- (1, "Parkett", 276, 10),
-- (1, "Parkett", 277, 10),
-- (1, "Parkett", 278, 10),
-- (1, "Parkett", 279, 10),
-- (1, "Parkett", 280, 10),
-- -- Hovedscene Område '' Rad 11 
-- (1, "Parkett", 281, 11), 
-- (1, "Parkett", 282, 11), 
-- (1, "Parkett", 283, 11), 
-- (1, "Parkett", 284, 11), 
-- (1, "Parkett", 285, 11), 
-- (1, "Parkett", 286, 11), 
-- (1, "Parkett", 287, 11), 
-- (1, "Parkett", 288, 11), 
-- (1, "Parkett", 289, 11), 
-- (1, "Parkett", 290, 11), 
-- (1, "Parkett", 291, 11), 
-- (1, "Parkett", 292, 11), 
-- (1, "Parkett", 293, 11), 
-- (1, "Parkett", 294, 11), 
-- (1, "Parkett", 295, 11), 
-- (1, "Parkett", 296, 11), 
-- (1, "Parkett", 297, 11), 
-- (1, "Parkett", 298, 11), 
-- -- Hovedscene Område '' Rad 12 
-- (1, "Parkett", 309, 12), 
-- (1, "Parkett", 310, 12), 
-- (1, "Parkett", 311, 12), 
-- (1, "Parkett", 312, 12), 
-- (1, "Parkett", 313, 12), 
-- (1, "Parkett", 314, 12), 
-- (1, "Parkett", 315, 12), 
-- (1, "Parkett", 316, 12), 
-- (1, "Parkett", 317, 12), 
-- (1, "Parkett", 318, 12), 
-- (1, "Parkett", 319, 12), 
-- (1, "Parkett", 320, 12), 
-- (1, "Parkett", 321, 12), 
-- (1, "Parkett", 322, 12), 
-- (1, "Parkett", 323, 12), 
-- (1, "Parkett", 324, 12), 
-- (1, "Parkett", 325, 12), 
-- (1, "Parkett", 326, 12), 
-- -- Hovedscene Område 'Parkett' Rad 13 
-- (1, "Parkett", 337, 13), 
-- (1, "Parkett", 338, 13), 
-- (1, "Parkett", 339, 13), 
-- (1, "Parkett", 340, 13), 
-- (1, "Parkett", 341, 13), 
-- (1, "Parkett", 342, 13), 
-- (1, "Parkett", 343, 13), 
-- (1, "Parkett", 344, 13), 
-- (1, "Parkett", 345, 13), 
-- (1, "Parkett", 346, 13), 
-- (1, "Parkett", 347, 13), 
-- (1, "Parkett", 348, 13), 
-- (1, "Parkett", 349, 13), 
-- (1, "Parkett", 350, 13), 
-- (1, "Parkett", 351, 13), 
-- (1, "Parkett", 352, 13), 
-- (1, "Parkett", 353, 13), 
-- (1, "Parkett", 354, 13), 
-- -- Hovedscene Område 'Parkett' Rad 14 
-- (1, "Parkett", 365, 14), 
-- (1, "Parkett", 366, 14), 
-- (1, "Parkett", 367, 14), 
-- (1, "Parkett", 368, 14), 
-- (1, "Parkett", 369, 14), 
-- (1, "Parkett", 370, 14), 
-- (1, "Parkett", 371, 14), 
-- (1, "Parkett", 372, 14), 
-- (1, "Parkett", 373, 14), 
-- (1, "Parkett", 374, 14), 
-- (1, "Parkett", 375, 14), 
-- (1, "Parkett", 376, 14), 
-- (1, "Parkett", 377, 14), 
-- (1, "Parkett", 378, 14), 
-- (1, "Parkett", 379, 14), 
-- (1, "Parkett", 380, 14), 
-- (1, "Parkett", 381, 14), 
-- (1, "Parkett", 382, 14), 
-- -- Hovedscene Område 'Parkett' Rad 15 
-- (1, "Parkett", 393, 15), 
-- (1, "Parkett", 394, 15), 
-- (1, "Parkett", 395, 15), 
-- (1, "Parkett", 396, 15), 
-- (1, "Parkett", 397, 15), 
-- (1, "Parkett", 398, 15), 
-- (1, "Parkett", 399, 15), 
-- (1, "Parkett", 400, 15), 
-- (1, "Parkett", 401, 15), 
-- (1, "Parkett", 402, 15), 
-- (1, "Parkett", 403, 15), 
-- (1, "Parkett", 404, 15), 
-- (1, "Parkett", 405, 15), 
-- (1, "Parkett", 406, 15), 
-- (1, "Parkett", 407, 15), 
-- (1, "Parkett", 408, 15), 
-- (1, "Parkett", 409, 15), 
-- (1, "Parkett", 410, 15), 
-- (1, "Parkett", 411, 15), 
-- (1, "Parkett", 412, 15), 
-- (1, "Parkett", 413, 15), 
-- (1, "Parkett", 414, 15), 
-- (1, "Parkett", 415, 15), 
-- (1, "Parkett", 416, 15), 
-- (1, "Parkett", 417, 15), 
-- (1, "Parkett", 418, 15), 
-- (1, "Parkett", 419, 15), 
-- (1, "Parkett", 420, 15),
-- -- Hovedscene Område 'Parkett' Rad 16
-- (1, "Parkett", 421, 16), 
-- (1, "Parkett", 422, 16), 
-- (1, "Parkett", 423, 16), 
-- (1, "Parkett", 424, 16), 
-- (1, "Parkett", 425, 16), 
-- (1, "Parkett", 426, 16), 
-- (1, "Parkett", 427, 16), 
-- (1, "Parkett", 428, 16), 
-- (1, "Parkett", 429, 16), 
-- (1, "Parkett", 430, 16), 
-- (1, "Parkett", 431, 16), 
-- (1, "Parkett", 432, 16), 
-- (1, "Parkett", 433, 16), 
-- (1, "Parkett", 434, 16), 
-- (1, "Parkett", 435, 16), 
-- (1, "Parkett", 436, 16), 
-- (1, "Parkett", 437, 16), 
-- (1, "Parkett", 438, 16), 
-- (1, "Parkett", 439, 16), 
-- (1, "Parkett", 440, 16), 
-- (1, "Parkett", 441, 16), 
-- (1, "Parkett", 442, 16), 
-- (1, "Parkett", 443, 16), 
-- (1, "Parkett", 444, 16), 
-- (1, "Parkett", 445, 16), 
-- (1, "Parkett", 446, 16), 
-- (1, "Parkett", 447, 16), 
-- (1, "Parkett", 448, 16),
-- --Hovedscene Område 'Parkett' Rad 17
-- (1, "Parkett", 449, 17), 
-- (1, "Parkett", 450, 17), 
-- (1, "Parkett", 451, 17), 
-- (1, "Parkett", 452, 17), 
-- (1, "Parkett", 453, 17), 
-- (1, "Parkett", 454, 17), 
-- (1, "Parkett", 455, 17), 
-- (1, "Parkett", 456, 17), 
-- (1, "Parkett", 457, 17), 
-- (1, "Parkett", 458, 17),
-- (1, "Parkett", 459, 17), 
-- (1, "Parkett", 460, 17), 
-- (1, "Parkett", 461, 17), 
-- (1, "Parkett", 462, 17), 
-- (1, "Parkett", 463, 17), 
-- (1, "Parkett", 464, 17), 
-- (1, "Parkett", 465, 17), 
-- (1, "Parkett", 466, 17), 
-- (1, "Parkett", 471, 17), 
-- (1, "Parkett", 472, 17), 
-- (1, "Parkett", 473, 17), 
-- (1, "Parkett", 474, 17), 
-- (1, "Parkett", 475, 17), 
-- (1, "Parkett", 476, 17),
-- --Hovedscene Område 'Parkett' Rad 18
-- (1, "Parkett", 477, 18), 
-- (1, "Parkett", 478, 18), 
-- (1, "Parkett", 479, 18), 
-- (1, "Parkett", 480, 18), 
-- (1, "Parkett", 481, 18), 
-- (1, "Parkett", 482, 18), 
-- (1, "Parkett", 483, 18), 
-- (1, "Parkett", 484, 18), 
-- (1, "Parkett", 485, 18), 
-- (1, "Parkett", 486, 18),
-- (1, "Parkett", 487, 18), 
-- (1, "Parkett", 488, 18), 
-- (1, "Parkett", 489, 18), 
-- (1, "Parkett", 490, 18), 
-- (1, "Parkett", 491, 18), 
-- (1, "Parkett", 492, 18), 
-- (1, "Parkett", 493, 18), 
-- (1, "Parkett", 494, 18), 
-- (1, "Parkett", 499, 18), 
-- (1, "Parkett", 500, 18),
-- (1, "Parkett", 501, 18), 
-- (1, "Parkett", 502, 18), 
-- (1, "Parkett", 503, 18), 
-- (1, "Parkett", 504, 18), 
-- --Hovedscene Område 'Galleri Nedre'
-- (1, "Galleri", 505, 1), 
-- (1, "Galleri", 506, 1), 
-- (1, "Galleri", 507, 1), 
-- (1, "Galleri", 508, 1), 
-- (1, "Galleri", 509, 1), 
-- --Hovedscene Område 'Galleri Nedre'
-- (1, "Galleri", 510, 2), 
-- (1, "Galleri", 511, 2), 
-- (1, "Galleri", 512, 2), 
-- (1, "Galleri", 513, 2), 
-- (1, "Galleri", 514, 2), 
-- --Hovedscene Område 'Galleri Øvre'
-- (1, "Galleri", 515, 3), 
-- (1, "Galleri", 516, 3), 
-- (1, "Galleri", 517, 3), 
-- (1, "Galleri", 518, 3), 
-- (1, "Galleri", 519, 3), 
-- --Hovedscene Område 'Galleri Øvre'
-- (1, "Galleri", 520, 4), 
-- (1, "Galleri", 521, 4), 
-- (1, "Galleri", 522, 4), 
-- (1, "Galleri", 523, 4), 
-- (1, "Galleri", 524, 4),
-- --Gamlescene Område 'Galleri' Rad 1
-- (2, "Galleri", 1, 1),
-- (2, "Galleri", 2, 1),
-- (2, "Galleri", 3, 1),
-- (2, "Galleri", 4, 1),
-- (2, "Galleri", 5, 1),
-- (2, "Galleri", 6, 1),
-- (2, "Galleri", 7, 1),
-- (2, "Galleri", 8, 1),
-- (2, "Galleri", 9, 1),
-- (2, "Galleri", 10, 1),
-- (2, "Galleri", 11, 1),
-- (2, "Galleri", 12, 1),
-- (2, "Galleri", 13, 1),
-- (2, "Galleri", 14, 1),
-- (2, "Galleri", 15, 1),
-- (2, "Galleri", 16, 1),
-- (2, "Galleri", 17, 1),
-- (2, "Galleri", 18, 1),
-- (2, "Galleri", 19, 1),
-- (2, "Galleri", 20, 1),
-- (2, "Galleri", 21, 1),
-- (2, "Galleri", 22, 1),
-- (2, "Galleri", 23, 1),
-- (2, "Galleri", 24, 1),
-- (2, "Galleri", 25, 1),
-- (2, "Galleri", 26, 1),
-- (2, "Galleri", 27, 1),
-- (2, "Galleri", 28, 1),
-- (2, "Galleri", 29, 1),
-- (2, "Galleri", 30, 1),
-- (2, "Galleri", 31, 1),
-- (2, "Galleri", 32, 1),
-- (2, "Galleri", 33, 1),
-- --Gamlescene Område 'Galleri' Rad 2
-- (2, "Galleri", 1, 2),
-- (2, "Galleri", 2, 2),
-- (2, "Galleri", 3, 2),
-- (2, "Galleri", 4, 2),
-- (2, "Galleri", 5, 2),
-- (2, "Galleri", 6, 2),
-- (2, "Galleri", 7, 2),
-- (2, "Galleri", 8, 2),
-- (2, "Galleri", 9, 2),
-- (2, "Galleri", 10, 2),
-- (2, "Galleri", 11, 2),
-- (2, "Galleri", 12, 2),
-- (2, "Galleri", 13, 2),
-- (2, "Galleri", 14, 2),
-- (2, "Galleri", 15, 2),
-- (2, "Galleri", 16, 2),
-- (2, "Galleri", 17, 2),
-- (2, "Galleri", 18, 2),
-- --Gamlescene Område 'Galleri' Rad 3
-- (2, "Galleri", 1, 3),
-- (2, "Galleri", 2, 3),
-- (2, "Galleri", 3, 3),
-- (2, "Galleri", 4, 3),
-- (2, "Galleri", 5, 3),
-- (2, "Galleri", 6, 3),
-- (2, "Galleri", 7, 3),
-- (2, "Galleri", 8, 3),
-- (2, "Galleri", 9, 3),
-- (2, "Galleri", 10, 3),
-- (2, "Galleri", 11, 3),
-- (2, "Galleri", 12, 3),
-- (2, "Galleri", 13, 3),
-- (2, "Galleri", 14, 3),
-- (2, "Galleri", 15, 3),
-- (2, "Galleri", 16, 3),
-- (2, "Galleri", 17, 3),
-- --Gamlescene Område 'Balkong' Rad 1
-- (2, "Balkong", 1, 1),
-- (2, "Balkong", 2, 1),
-- (2, "Balkong", 3, 1),
-- (2, "Balkong", 4, 1),
-- (2, "Balkong", 5, 1),
-- (2, "Balkong", 6, 1),
-- (2, "Balkong", 7, 1),
-- (2, "Balkong", 8, 1),
-- (2, "Balkong", 9, 1),
-- (2, "Balkong", 10, 1),
-- (2, "Balkong", 11, 1),
-- (2, "Balkong", 12, 1),
-- (2, "Balkong", 13, 1),
-- (2, "Balkong", 14, 1),
-- (2, "Balkong", 15, 1),
-- (2, "Balkong", 16, 1),
-- (2, "Balkong", 17, 1),
-- (2, "Balkong", 18, 1),
-- (2, "Balkong", 19, 1),
-- (2, "Balkong", 20, 1),
-- (2, "Balkong", 21, 1),
-- (2, "Balkong", 22, 1),
-- (2, "Balkong", 23, 1),
-- (2, "Balkong", 24, 1),
-- (2, "Balkong", 25, 1),
-- (2, "Balkong", 26, 1),
-- (2, "Balkong", 27, 1),
-- (2, "Balkong", 28, 1),
-- --Gamlescene Område 'Balkong' Rad 2
-- (2, "Balkong", 1, 2),
-- (2, "Balkong", 2, 2),
-- (2, "Balkong", 3, 2),
-- (2, "Balkong", 4, 2),
-- (2, "Balkong", 5, 2),
-- (2, "Balkong", 6, 2),
-- (2, "Balkong", 7, 2),
-- (2, "Balkong", 8, 2),
-- (2, "Balkong", 9, 2),
-- (2, "Balkong", 10, 2),
-- (2, "Balkong", 11, 2),
-- (2, "Balkong", 12, 2),
-- (2, "Balkong", 13, 2),
-- (2, "Balkong", 14, 2),
-- (2, "Balkong", 15, 2),
-- (2, "Balkong", 16, 2),
-- (2, "Balkong", 17, 2),
-- (2, "Balkong", 18, 2),
-- (2, "Balkong", 19, 2),
-- (2, "Balkong", 20, 2),
-- (2, "Balkong", 21, 2),
-- (2, "Balkong", 22, 2),
-- (2, "Balkong", 23, 2),
-- (2, "Balkong", 24, 2),
-- (2, "Balkong", 25, 2),
-- (2, "Balkong", 26, 2),
-- (2, "Balkong", 27, 2),
-- --Gamlescene Område 'Balkong' Rad 3
-- (2, "Balkong", 1, 3),
-- (2, "Balkong", 2, 3),
-- (2, "Balkong", 3, 3),
-- (2, "Balkong", 4, 3),
-- (2, "Balkong", 5, 3),
-- (2, "Balkong", 6, 3),
-- (2, "Balkong", 7, 3),
-- (2, "Balkong", 8, 3),
-- (2, "Balkong", 9, 3),
-- (2, "Balkong", 10, 3),
-- (2, "Balkong", 11, 3),
-- (2, "Balkong", 12, 3),
-- (2, "Balkong", 13, 3),
-- (2, "Balkong", 14, 3),
-- (2, "Balkong", 15, 3),
-- (2, "Balkong", 16, 3),
-- (2, "Balkong", 17, 3),
-- (2, "Balkong", 18, 3),
-- (2, "Balkong", 19, 3),
-- (2, "Balkong", 20, 3),
-- (2, "Balkong", 21, 3),
-- (2, "Balkong", 22, 3),
-- --Gamlescene Område 'Balkong' Rad 4
-- (2, "Balkong", 1, 4),
-- (2, "Balkong", 2, 4),
-- (2, "Balkong", 3, 4),
-- (2, "Balkong", 4, 4),
-- (2, "Balkong", 5, 4),
-- (2, "Balkong", 6, 4),
-- (2, "Balkong", 7, 4),
-- (2, "Balkong", 8, 4),
-- (2, "Balkong", 9, 4),
-- (2, "Balkong", 10, 4),
-- (2, "Balkong", 11, 4),
-- (2, "Balkong", 12, 4),
-- (2, "Balkong", 13, 4),
-- (2, "Balkong", 14, 4),
-- (2, "Balkong", 15, 4),
-- (2, "Balkong", 16, 4),
-- (2, "Balkong", 17, 4),
-- --Gamlescene Område 'Parkett' Rad 1
-- (2, "Parkett", 1, 1),
-- (2, "Parkett", 2, 1),
-- (2, "Parkett", 3, 1),
-- (2, "Parkett", 4, 1),
-- (2, "Parkett", 5, 1),
-- (2, "Parkett", 6, 1),
-- (2, "Parkett", 7, 1),
-- (2, "Parkett", 8, 1),
-- (2, "Parkett", 9, 1),
-- (2, "Parkett", 10, 1),
-- (2, "Parkett", 11, 1),
-- (2, "Parkett", 12, 1),
-- (2, "Parkett", 13, 1),
-- (2, "Parkett", 14, 1),
-- (2, "Parkett", 15, 1),
-- (2, "Parkett", 16, 1),
-- (2, "Parkett", 17, 1),
-- (2, "Parkett", 18, 1),
-- --Gamlescene Område 'Parkett' Rad 2
-- (2, "Parkett", 1, 2),
-- (2, "Parkett", 2, 2),
-- (2, "Parkett", 3, 2),
-- (2, "Parkett", 4, 2),
-- (2, "Parkett", 5, 2),
-- (2, "Parkett", 6, 2),
-- (2, "Parkett", 7, 2),
-- (2, "Parkett", 8, 2),
-- (2, "Parkett", 9, 2),
-- (2, "Parkett", 10, 2),
-- (2, "Parkett", 11, 2),
-- (2, "Parkett", 12, 2),
-- (2, "Parkett", 13, 2),
-- (2, "Parkett", 14, 2),
-- (2, "Parkett", 15, 2),
-- (2, "Parkett", 16, 2),
-- --Gamlescene Område 'Parkett' Rad 3
-- (2, "Parkett", 1, 3),
-- (2, "Parkett", 2, 3),
-- (2, "Parkett", 3, 3),
-- (2, "Parkett", 4, 3),
-- (2, "Parkett", 5, 3),
-- (2, "Parkett", 6, 3),
-- (2, "Parkett", 7, 3),
-- (2, "Parkett", 8, 3),
-- (2, "Parkett", 9, 3),
-- (2, "Parkett", 10, 3),
-- (2, "Parkett", 11, 3),
-- (2, "Parkett", 12, 3),
-- (2, "Parkett", 13, 3),
-- (2, "Parkett", 14, 3),
-- (2, "Parkett", 15, 3),
-- (2, "Parkett", 16, 3),
-- (2, "Parkett", 17, 3),
-- --Gamlescene Område 'Parkett' Rad 4
-- (2, "Parkett", 1, 4),
-- (2, "Parkett", 2, 4),
-- (2, "Parkett", 3, 4),
-- (2, "Parkett", 4, 4),
-- (2, "Parkett", 5, 4),
-- (2, "Parkett", 6, 4),
-- (2, "Parkett", 7, 4),
-- (2, "Parkett", 8, 4),
-- (2, "Parkett", 9, 4),
-- (2, "Parkett", 10, 4),
-- (2, "Parkett", 11, 4),
-- (2, "Parkett", 12, 4),
-- (2, "Parkett", 13, 4),
-- (2, "Parkett", 14, 4),
-- (2, "Parkett", 15, 4),
-- (2, "Parkett", 16, 4),
-- (2, "Parkett", 17, 4),
-- (2, "Parkett", 18, 4),
-- --Gamlescene Område 'Parkett' Rad 5
-- (2, "Parkett", 1, 5),
-- (2, "Parkett", 2, 5),
-- (2, "Parkett", 3, 5),
-- (2, "Parkett", 4, 5),
-- (2, "Parkett", 5, 5),
-- (2, "Parkett", 6, 5),
-- (2, "Parkett", 7, 5),
-- (2, "Parkett", 8, 5),
-- (2, "Parkett", 9, 5),
-- (2, "Parkett", 10, 5),
-- (2, "Parkett", 11, 5),
-- (2, "Parkett", 12, 5),
-- (2, "Parkett", 13, 5),
-- (2, "Parkett", 14, 5),
-- (2, "Parkett", 15, 5),
-- (2, "Parkett", 16, 5),
-- (2, "Parkett", 17, 5),
-- (2, "Parkett", 18, 5),
-- --Gamlescene Område 'Parkett' Rad 6
-- (2, "Parkett", 1, 6),
-- (2, "Parkett", 2, 6),
-- (2, "Parkett", 3, 6),
-- (2, "Parkett", 4, 6),
-- (2, "Parkett", 5, 6),
-- (2, "Parkett", 6, 6),
-- (2, "Parkett", 7, 6),
-- (2, "Parkett", 8, 6),
-- (2, "Parkett", 9, 6),
-- (2, "Parkett", 10, 6),
-- (2, "Parkett", 11, 6),
-- (2, "Parkett", 12, 6),
-- (2, "Parkett", 13, 6),
-- (2, "Parkett", 14, 6),
-- (2, "Parkett", 15, 6),
-- (2, "Parkett", 16, 6),
-- (2, "Parkett", 17, 6),
-- --Gamlescene Område 'Parkett' Rad 7
-- (2, "Parkett", 1, 7),
-- (2, "Parkett", 2, 7),
-- (2, "Parkett", 3, 7),
-- (2, "Parkett", 4, 7),
-- (2, "Parkett", 5, 7),
-- (2, "Parkett", 6, 7),
-- (2, "Parkett", 7, 7),
-- (2, "Parkett", 8, 7),
-- (2, "Parkett", 9, 7),
-- (2, "Parkett", 10, 7),
-- (2, "Parkett", 11, 7),
-- (2, "Parkett", 12, 7),
-- (2, "Parkett", 13, 7),
-- (2, "Parkett", 14, 7),
-- (2, "Parkett", 15, 7),
-- (2, "Parkett", 16, 7),
-- (2, "Parkett", 17, 7),
-- (2, "Parkett", 18, 7),
-- --Gamlescene Område 'Parkett' Rad 8
-- (2, "Parkett", 1, 8),
-- (2, "Parkett", 2, 8),
-- (2, "Parkett", 3, 8),
-- (2, "Parkett", 4, 8),
-- (2, "Parkett", 5, 8),
-- (2, "Parkett", 6, 8),
-- (2, "Parkett", 7, 8),
-- (2, "Parkett", 8, 8),
-- (2, "Parkett", 9, 8),
-- (2, "Parkett", 10, 8),
-- (2, "Parkett", 11, 8),
-- (2, "Parkett", 12, 8),
-- (2, "Parkett", 13, 8),
-- (2, "Parkett", 14, 8),
-- (2, "Parkett", 15, 8),
-- (2, "Parkett", 16, 8),
-- (2, "Parkett", 17, 8),
-- --Gamlescene Område 'Parkett' Rad 9
-- (2, "Parkett", 1, 9),
-- (2, "Parkett", 2, 9),
-- (2, "Parkett", 3, 9),
-- (2, "Parkett", 4, 9),
-- (2, "Parkett", 5, 9),
-- (2, "Parkett", 6, 9),
-- (2, "Parkett", 7, 9),
-- (2, "Parkett", 8, 9),
-- (2, "Parkett", 9, 9),
-- (2, "Parkett", 10, 9),
-- (2, "Parkett", 11, 9),
-- (2, "Parkett", 12, 9),
-- (2, "Parkett", 13, 9),
-- (2, "Parkett", 14, 9),
-- (2, "Parkett", 15, 9),
-- (2, "Parkett", 16, 9),
-- (2, "Parkett", 17, 9),
-- --Gamlescene Område 'Parkett' Rad 10
-- (2, "Parkett", 1, 10),
-- (2, "Parkett", 2, 10),
-- (2, "Parkett", 3, 10),
-- (2, "Parkett", 4, 10),
-- (2, "Parkett", 5, 10),
-- (2, "Parkett", 6, 10),
-- (2, "Parkett", 7, 10),
-- (2, "Parkett", 8, 10),
-- (2, "Parkett", 9, 10),
-- (2, "Parkett", 10, 10),
-- (2, "Parkett", 11, 10),
-- (2, "Parkett", 12, 10),
-- (2, "Parkett", 13, 10),
-- (2, "Parkett", 14, 10);